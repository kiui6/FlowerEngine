...
class LinuxMappedFile : public MappedFile
{
    int m_fd;
protected:
    size_t m_size = 0;
    ...
    LinuxMappedFile() = default;
    LinuxMappedFile(const std::string& path, FileAccess access) { Open(path, access); }
    friend class LinuxPlatformFilesystem;
public:
    ~LinuxMappedFile();
